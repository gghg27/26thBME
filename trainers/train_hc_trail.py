...
核心训练目标：
    loss = lambda_emo * CE(emo_logits, emotion_label)
         + lambda_con * TrialCompact(contrast_feat, subject_id, trial_id)
...
from models.hc_trail_con import EmotionPretrainModel
...
def trial_compactness_loss(
    ...
    This replaces SupCon: there are no label positives and no explicit negatives.
    """
    if features.ndim != 2:
...
def train_one_epoch_emotion_grl(
    ...
    for batch in pbar:
        ...
        if lambda_con > 0:
            ...
            loss_con = trial_compactness_loss(contrast_feat, trial_group_ids)
        else:
    ...
    for repeat, rand in enumerate(config.HC_REPEAT_SEEDS):
        ...
        for fold in range(n_splits):
            ...
            result = train_competition_cross_subject(
                ...
                fold=fold,
                save_dir=os.path.join(model_params_root, f"hc_trail_reapt{repeat}_fold{fold}"),
                dataset="com",
                train_group=train_group,
                n_splits=n_splits,
                epochs=100,
                ...
                # trial compactness：同一个 subject-trial 内的窗口表征尽量接近
                lambda_con=0.1,
                con_temperature=0.1,
