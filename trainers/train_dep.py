...
def save_best_checkpoint(
    ...
    model_file = "dep_best.pt" if best_name == "combined" else f"best_{best_name}_fold{fold}.pt"
    ...
    for repeat, rand in enumerate(config.DEP_REPEAT_SEEDS):
        ...
        for fold in range(n_splits):
            ...
            result = train_competition_cross_subject(
                ...
                fold=fold,
                save_dir=os.path.join(model_params_root, f"dep_reapt{repeat}_fold{fold}"),
                dataset="com",
                train_group=train_group,
                n_splits=n_splits,
                epochs=100,
