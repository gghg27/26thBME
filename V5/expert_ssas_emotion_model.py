...
class Stage2ExpertEmotionAdaptationModel(nn.Module):
    ...
    def __init__(
        ...
        shared_expert_trunk: bool = False,
        ...
        self.shared_expert_trunk = bool(shared_expert_trunk)
        ...
        if self.shared_expert_trunk:
            ...
            self.hc_classifier = nn.Linear(self.expert_hidden_dim, emotion_classes)
...
def weighted_mmd_rbf(
    ...
    """RBF MMD with optional normalized weights on both source and target."""

    if x.size(0) == 0 or y.size(0) == 0:
        return x.new_tensor(0.0)

    def _normalize_weight(weight: Optional[torch.Tensor], n: int, ref: torch.Tensor) -> Optional[torch.Tensor]:
        if weight is None:
            return torch.full((n,), 1.0 / n, device=ref.device, dtype=ref.dtype)
        w = weight.to(device=ref.device, dtype=ref.dtype).flatten().clamp_min(0.0)
        if w.numel() != n:
            raise ValueError(f"Expected weight shape [{n}], got {tuple(w.shape)}")
        w_sum = w.sum()
        if w_sum <= eps:
            return None
        return w / w_sum
    ...
    wx = _normalize_weight(source_weight, ns, x)
    wy = _normalize_weight(target_weight, nt, y)
    if wx is None or wy is None:
        return x.new_tensor(0.0)

    loss_ss = (wx[:, None] * wx[None, :] * k_ss).sum()
    loss_tt = (wy[:, None] * wy[None, :] * k_tt).sum()
    loss_st = (wx[:, None] * wy[None, :] * k_st).sum()
    return torch.clamp(loss_ss + loss_tt - 2.0 * loss_st, min=0.0)


def soft_conditional_mmd_rbf(
    z_source: torch.Tensor,
    z_target: torch.Tensor,
    y_source: torch.Tensor,
    target_prob: torch.Tensor,
    source_sample_weight: Optional[torch.Tensor] = None,
    num_classes: int = 2,
    conf_threshold: float = 0.0,
    detach_target_prob: bool = True,
    min_weight: float = 1e-6,
    kernel_mul: float = 2.0,
    kernel_num: int = 5,
    fix_sigma: Optional[float] = None,
) -> tuple[torch.Tensor, dict]:
    """Soft class-wise conditional MMD using source labels and target probabilities."""

    if detach_target_prob:
        target_prob = target_prob.detach()

    y_source = y_source.reshape(-1).long()
    target_prob = target_prob.to(device=z_target.device, dtype=z_target.dtype)
    conf = target_prob.max(dim=1).values if target_prob.numel() else target_prob.new_zeros((0,))
    conf_mask = (
        (conf >= float(conf_threshold)).to(dtype=target_prob.dtype)
        if conf_threshold > 0
        else torch.ones_like(conf, dtype=target_prob.dtype)
    )

    stats = {
        "cond_mmd_valid_classes": 0,
        "cond_mmd_target_conf_mean": float(conf.detach().mean().item()) if conf.numel() else 0.0,
        "cond_mmd_target_conf_ratio": float(conf_mask.detach().mean().item()) if conf_mask.numel() else 0.0,
        "cond_mmd_target_pseudo_pos_rate": (
            float(target_prob[:, 1].detach().mean().item())
            if target_prob.size(0) > 0 and target_prob.size(1) > 1
            else 0.0
        ),
    }
    ...
    source_sample_weight = (
        source_sample_weight.to(device=z_source.device, dtype=z_source.dtype).reshape(-1)
        if source_sample_weight is not None
        else None
    )

    for cls in range(int(num_classes)):
        source_class_weight = (y_source == cls).to(device=z_source.device, dtype=z_source.dtype)
        if source_sample_weight is not None:
            source_class_weight = source_class_weight * source_sample_weight

        if cls < target_prob.size(1):
            target_class_weight = target_prob[:, cls] * conf_mask
        else:
            target_class_weight = target_prob.new_zeros((target_prob.size(0),))

        source_weight_sum = source_class_weight.detach().sum()
        target_weight_sum = target_class_weight.detach().sum()
        stats[f"cond_mmd_source_class{cls}_weight"] = float(source_weight_sum.item())
        stats[f"cond_mmd_target_class{cls}_weight"] = float(target_weight_sum.item())
    ...
    if not class_losses:
        return z_source.new_tensor(0.0), stats

    stats["cond_mmd_valid_classes"] = int(len(class_losses))
    return torch.stack(class_losses).mean(), stats
