...
from V8.spatial_cheb_conv import SpatialAttentionChebConv
...
class RegionPLVPMGEncoder(nn.Module):
    ...
    结构:
        Local spatial-attention Chebyshev graph convolution (channel level)
        + Region PLV-GCN (脑区级)
        + Fusion

    输入:
        node_features: [B, 30, 64]
        plv_adj:       [B, 30, 30], Top-K sparse and not normalized
        plv_matrix:    [B, 30, 30]
    ...
    def __init__(
        ...
        # ---- 1. Local order-2 Chebyshev convolution ----
        self.local_cheb_gcn = SpatialAttentionChebConv(
            in_dim=node_dim, out_dim=hidden_dim, dropout=dropout, gamma_init=cheb_gamma_init,
        )
        # Input node_features + unnormalized Top-K PLV + trial spatial attention.
    ...
    def forward(
            ...
            plv_adj: torch.Tensor,
            plv_matrix: torch.Tensor,
            spatial_attention: torch.Tensor,
            return_graph_debug: bool = False,
        ...
        """
        # ---- 1. Local spatial-attention Chebyshev convolution ----
        # local_node_embeddings: [B, 30, 64]
        cheb = self.local_cheb_gcn(node_features, plv_adj, spatial_attention, return_graph_debug)
        local_node_embeddings = cheb["output"]
        ...
        result = {
            ...
            "cheb_gamma": cheb["gamma"],
        }
        if return_graph_debug:
            result.update({"cheb_effective_adj": cheb["effective_adj"],
                           "cheb_laplacian": cheb["laplacian"],
                           "cheb_laplacian_tilde": cheb["laplacian_tilde"],
                           "cheb_t0": cheb["t0"], "cheb_t1": cheb["t1"], "cheb_t2": cheb["t2"]})
        return result
...
class BrainGraphBackbone(nn.Module):
    ...
    def __init__(
        ...
        self.cheb_order, self.cheb_num_terms = 2, 3
        self.cheb_gamma_init = float(cheb_gamma_init)
        self.use_de_graph_diagonal = False
    ...
    def _forward_dual_head(
        ...
        if spatial_attention is None:
            raise ValueError("V8 BrainGraphBackbone requires trial-level spatial_attention")
        spatial_attention = torch.nan_to_num(spatial_attention, nan=0.0, posinf=0.0, neginf=0.0)
        # Kept only as compatibility outputs. DE graph-diagonal modules are not called in V8.
        power_diag_emotion = de_feat.new_zeros(de_feat.shape[:2])
        power_diag_diag = de_feat.new_zeros(de_feat.shape[:2])
        band_weight_emotion = de_feat.new_zeros(self.de_num_bands)
        band_weight_diag = de_feat.new_zeros(self.de_num_bands)
        unit_diag = de_feat.new_ones(de_feat.shape[:2])

        plv_graph_emotion = self.plv_graph_constructor(
            x,
            diag_values=unit_diag,
        )
        ...
        pmg_emotion = self.pmg_encoder(
            ...
            plv_adj=plv_graph_emotion["adj_masked"],
            plv_matrix=plv_graph_emotion["plv_matrix"],
            spatial_attention=spatial_attention,
            return_graph_debug=return_graph_debug,
        )
        pmg_diag = self.pmg_encoder(
            node_features=node_features_diag,
            plv_adj=plv_graph_diag["adj_masked"],
        ...
        return {
            ...
            "cheb_gamma": pmg_emotion["cheb_gamma"],
            "cheb_gamma_diag": pmg_diag["cheb_gamma"],
            "cheb_effective_adj": pmg_emotion.get("cheb_effective_adj"),
            "cheb_effective_adj_diag": pmg_diag.get("cheb_effective_adj"),
            "cheb_laplacian": pmg_emotion.get("cheb_laplacian"),
            "cheb_laplacian_diag": pmg_diag.get("cheb_laplacian"),
            "cheb_t0": pmg_emotion.get("cheb_t0"),
            "cheb_t1": pmg_emotion.get("cheb_t1"),
            "cheb_t2": pmg_emotion.get("cheb_t2"),
