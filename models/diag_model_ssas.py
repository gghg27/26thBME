...
class DomainDiscriminator(nn.Module):
    """Small MLP domain discriminator used by SSAS."""
...
class SourceSelectionModel(nn.Module):
    ...
    Stage 1:
        ...
        C_ss = diagnosis classifier
        D_ss = multi-source + target domain classifier

    D_ss has K+1 outputs:
        0..K-1: source subject domains
        K: target domain
    """
    ...
    def forward(
        ...
        z = self.extract_features(x, de_feat)
        diagnosis_logits = self.classifier(z)
...
class SSASDiagnosisModel(nn.Module):
    """
    Stage 2 / Stage 3 final model:
        F = BrainGraphBackbone
        C = diagnosis classifier
        D_as = trainable source-vs-target GRL discriminator
    """
