...
def flatten_trial_batch(batch: dict) -> tuple[dict, int, int]:
    ...
    for key in required:
        if key not in batch or not torch.is_tensor(batch[key]) or batch[key].dim() < 2:
            raise KeyError(f"Trial batch must contain tensor key {key!r} with [B,K,...].")
    bsz, num_windows = int(batch["x_abs"].shape[0]), int(batch["x_abs"].shape[1])
    if any(batch[key].shape[:2] != (bsz, num_windows) for key in required):
        raise AssertionError("dual-view trial tensors disagree on [B,K]")
    flat: dict[str, Any] = {}
