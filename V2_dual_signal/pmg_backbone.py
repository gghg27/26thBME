...
# ---------------------------------------------------------------------------
# Dual-signal V2 active backbone
# ---------------------------------------------------------------------------
# Keep the legacy class above only so old checkpoints/source references remain
# readable.  This later definition is the exported, active BrainGraphBackbone.
class BrainGraphBackbone(nn.Module):
    """One complete, single-view three-branch encoder.

    Absolute and relative signals are handled by calling this *same instance*
    twice in the stage models.  No feature-level subject baseline is accepted
    or computed here.
    """
    ...
    def __init__(
        ...
        if use_subject_relative_de or use_subject_relative_bio:
            raise ValueError(
                "Dual-signal backbone requires use_subject_relative_de=False "
                "and use_subject_relative_bio=False."
            )
        self.use_subject_relative_de = False
        self.use_subject_relative_bio = False
        self.de_num_bands = int(de_num_bands)
        self.node_encoder = NodeFeatureEncoder(
            sfreq=sfreq, temporal_dim=64, spectral_dim=16
        )
        if self.node_encoder.node_dim != node_dim:
            raise ValueError(
                f"Configured node_dim={node_dim}, encoder outputs "
                f"{self.node_encoder.node_dim}."
            )
        self.leranable_diag = LearnableDEDiagonalFusion(
            num_bands=self.de_num_bands
        )
        self.node_contrast_projector = NodeFeatureContrastProjector(
            node_dim=node_dim, hidden_dim=128, out_dim=64, dropout=dropout
        )
        ...
        self.plv_graph_constructor = RawSignalPLVGraphConstructor(
            num_nodes=30, topk=topk, symmetrize=True
        )
        self.pmg_encoder = RegionPLVPMGEncoder(
            node_dim=node_dim,
            hidden_dim=node_dim,
    ...
    def forward_single_view(
        self, x: torch.Tensor, de_feat: torch.Tensor
    ) -> Dict[str, torch.Tensor]:
        if x.ndim != 3:
            raise ValueError(f"x must be [B,C,T], got {tuple(x.shape)}")
        if de_feat.ndim != 3:
            raise ValueError(
                f"de_feat must be [B,C,{self.de_num_bands}], "
                f"got {tuple(de_feat.shape)}"
            )
        if x.shape[:2] != de_feat.shape[:2]:
            raise ValueError(
                f"x/de channel mismatch: {tuple(x.shape)} vs "
                f"{tuple(de_feat.shape)}"
            )
        ...
        diag_values, band_weight = self.leranable_diag(de_feat)
        plv = self.plv_graph_constructor(x, diag_values=diag_values)
        pmg = self.pmg_encoder(
            node_features=node_features,
            plv_adj_norm=plv["adj_norm"],
            plv_matrix=plv["plv_matrix"],
        )
        z_pmg = pmg["z_pmg"]
        z_core = torch.cat([z_conv, z_pmg], dim=-1)

        bio = {}
        if self.biomarker_extractor is not None:
            bio = self.biomarker_extractor(
                x,
                de_feat=de_feat,
                hjorth=hjorth,
                plv_matrix=plv["plv_matrix"],
        ...
        return {
            ...
            "node_features": node_features,
            "node_contrast_feat": node_contrast_feat,
            "node_embeddings": pmg["local_node_embeddings"],
            "pmg_node_embeddings": pmg["local_node_embeddings"],
            "local_node_embeddings": pmg["local_node_embeddings"],
            "z_local": pmg["z_local"],
            "z_region": pmg["z_region"],
            "region_features": pmg["region_features"],
            "region_adj": pmg["region_adj"],
            "region_adj_norm": pmg["region_adj_norm"],
            "region_embeddings": pmg["region_embeddings"],
            "adj_norm": plv["adj_norm"],
            "adj_masked": plv["adj_masked"],
            "adj_dense": plv["adj_dense"],
            "scores": plv["scores"],
            "plv_adj_norm": plv["adj_norm"],
            "plv_adj_masked": plv["adj_masked"],
            "plv_adj_dense": plv["adj_dense"],
            "plv_matrix": plv["plv_matrix"],
            "bio_feat": z_bio,
            "bio_raw": bio.get("bio_raw"),
            "bio_input": bio.get("bio_input"),
            "bio_freq_feat": bio.get("bio_freq_feat"),
            "bio_asym_feat": bio.get("bio_asym_feat"),
            "bio_hjorth_feat": bio.get("bio_hjorth_feat"),
            "bio_plv_feat": bio.get("bio_plv_feat"),
            "bio_nonlinear_feat": bio.get("bio_nonlinear_feat"),
            "power_diag_values": diag_values,
    ...
    def forward(
        ...
        return self.forward_single_view(x, de_feat)
...
class BrainGraphBackbone(BrainGraphBackbone):
    """Final exported class: force the single-view forward as the active path."""
