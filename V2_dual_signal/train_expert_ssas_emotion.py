...
import config
from V2_dual_signal.dataloader import (
    DualViewCompetitionDataset as Competition4ClassDataset,
    expand_dual_window_index,
    resolve_dual_path,
    validate_dual_index,
)
...
from V2_dual_signal.trial_supcon import (
...
def forward_dual(model: torch.nn.Module, batch: dict, **kwargs) -> dict:
    """The only model-call path used by train/validation/voting/test."""
    return model(
        x_abs=batch["x_abs"],
        de_abs=batch["de_abs"],
        x_rel=batch["x_rel"],
...
def check_dual_batch(batch: dict, prefix: str = "[DualView Check]") -> None:
    required = ("x_abs", "x_rel", "de_abs", "de_rel")
    for key in required:
        if key not in batch or not torch.is_tensor(batch[key]):
            raise KeyError(f"Missing tensor batch key: {key}")
        if not torch.isfinite(batch[key]).all():
            raise FloatingPointError(f"{key} contains NaN/Inf")
    if batch["x_abs"].shape != batch["x_rel"].shape:
        raise AssertionError("x_abs/x_rel shape mismatch")
    if batch["de_abs"].shape != batch["de_rel"].shape:
        raise AssertionError("de_abs/de_rel shape mismatch")
    print(prefix)
    for key in required:
        value = batch[key].float()
        print(f"  {key}: {list(value.shape)} mean={value.mean().item():.6g} std={value.std().item():.6g}")
    print(f"  mean abs signal difference: {(batch['x_abs'] - batch['x_rel']).abs().mean().item():.6g}")
    print(f"  mean abs DE difference: {(batch['de_abs'] - batch['de_rel']).abs().mean().item():.6g}")


def check_shared_encoder(model: torch.nn.Module, batch: Optional[dict] = None) -> None:
    if not hasattr(model, "shared_encoder"):
        raise AssertionError("model must expose exactly one shared_encoder")
    if hasattr(model, "encoder_abs") or hasattr(model, "encoder_rel"):
        raise AssertionError("encoder_abs/encoder_rel are forbidden")
    parameter_ids = tuple(id(parameter) for parameter in model.shared_encoder.parameters())
    if parameter_ids != tuple(id(parameter) for parameter in model.shared_encoder.parameters()):
        raise AssertionError("shared encoder parameter identity changed")
    observed: list[tuple[int, ...]] = []
    if batch is not None:
        handle = model.shared_encoder.register_forward_pre_hook(
            lambda module, _inputs: observed.append(tuple(id(p) for p in module.parameters()))
        )
        was_training = model.training
        model.eval()
        with torch.no_grad():
            forward_dual(model, batch)
        handle.remove()
        model.train(was_training)
        if len(observed) != 2 or observed[0] != observed[1] or observed[0] != parameter_ids:
            raise AssertionError("abs/rel forwards did not use identical encoder parameter ids")
    count = sum(parameter.numel() for parameter in model.shared_encoder.parameters())
    print(
        f"[Shared Encoder Check] parameter count={count:,}; one instance; "
        f"abs/rel calls={len(observed) if batch is not None else 'not-run'}; ids stable"
    )


def inspect_window_configuration(index_csv: str | Path, name: str) -> dict:
    df = pd.read_csv(index_csv)
    if "de_win_id" not in df.columns:
        df = expand_dual_window_index(df, root=ROOT)
    lengths = sorted((df["end"].astype(int) - df["start"].astype(int)).unique().tolist())
    trial_group = df.groupby(
        [column for column in ("subject_id", "user_id", "trial_id") if column in df.columns],
        dropna=False,
    )
    windows_per_trial = sorted(trial_group.size().unique().tolist())
    first_group = next(iter(trial_group))[1].sort_values("start")
    starts = first_group["start"].astype(int).to_numpy()
    steps = sorted(set(np.diff(starts).tolist())) if len(starts) > 1 else []
    de = np.load(resolve_dual_path(df.iloc[0]["de_path_abs"], ROOT), mmap_mode="r")
    result = {
    ...
    print(
        f"[Window Check:{name}] window_lengths={lengths}, steps={steps}, "
        f"DE shape={list(de.shape)}, windows_per_trial={windows_per_trial}"
    )
    return result
...
class DomainAwareCompetitionDataset(Competition4ClassDataset):
    ...
    def __init__(
        ...
        super().__init__(index_csv=index_csv, subject_ids=subject_ids, normalize=False)
...
class UnlabeledTargetDataset(Dataset):
    """Dual-view test windows; no window-level normalization is performed."""
    ...
    def __init__(
        ...
        if normalize:
            raise ValueError("Test window normalization is forbidden for dual-signal data.")
        raw_df = pd.read_csv(index_csv)
        self.id_col = "user_id" if "user_id" in raw_df.columns else "subject_id"
        self.df = expand_dual_window_index(raw_df, root=root)
        validate_dual_index(self.df, root=root)
        self.domain_mapping = domain_mapping
        self.root = Path(root)
        self.user_to_int = {
            uid: i
            for i, uid in enumerate(
                sorted(self.df[self.id_col].astype(str).unique(), key=natural_key)
            )
        }
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        de_abs = de_abs_seq[win_id].astype(np.float32, copy=False)
        de_rel = de_rel_seq[win_id].astype(np.float32, copy=False)
        if x_abs.shape != x_rel.shape or de_abs.shape != de_rel.shape:
            raise AssertionError("test dual-view shape mismatch")
        uid = str(row[self.id_col])
        domain_key = f"test:{uid}"
        subject_id = int(row.get("subject_number", self.user_to_int[uid]))
        return {
            ...
            "domain_id": torch.tensor(
                int(self.domain_mapping["key_to_domain"][domain_key]), dtype=torch.long
            ),
            "trial_id": torch.tensor(int(row["trial_id"]), dtype=torch.long),
            "user_id": uid,
            "target_key": domain_key,
...
def train_stage1_one_epoch(
    ...
    for source_batch in pbar:
        ...
        source_rel_kwargs = get_subject_relative_kwargs(
            source_batch,
            device,
            dtype=source_batch["de_rel"].dtype,
            de_mu=source_de_mu,
...
def validate_stage1(
    ...
    for _ in tqdm(range(steps), desc="Stage1-Val", leave=False):
        ...
        source_out = forward_dual(model, source_batch)
        target_out = forward_dual(model, target_batch)
...
def validate_stage2_trial_supcon(
    ...
    for batch in tqdm(val_loader, desc="Stage2-TrialSupCon-Val", leave=False):
        ...
        out = forward_dual(model, flat_batch, lambda_subject=0.0)
...
def validate_stage2_trial_level(
    ...
    for batch in tqdm(val_loader, desc="Stage2-Val", leave=False):
        ...
        out = forward_dual(model, batch, lambda_subject=0.0)
...
def run_one_fold(args, fold: int, repeat_index: int, rand_seed: int) -> dict:
    ...
    inspect_window_configuration(args.index_csv, "train")
    if args.test_csv and Path(args.test_csv).exists():
        inspect_window_configuration(args.test_csv, "test")
    ...
    use_subject_relative_bio = False
    print(
        "[Dual Signal] feature-level DE/Bio baselines disabled; "
        "relative features come only from x_rel/de_rel"
    )
    ...
    first_batch = move_batch_to_device(next(iter(source_loader)), device)
    check_dual_batch(first_batch)
    check_shared_encoder(stage1, first_batch)
    ...
    check_shared_encoder(stage2, first_batch)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--index_csv", type=str, default="data/com_index_dual_2s.csv")
    parser.add_argument("--test_csv", type=str, default="data/com_test_index_dual_2s.csv")
    parser.add_argument("--save_root", type=str, default="model_params/V2_dual_signal")
